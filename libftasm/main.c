...
/*   Updated: 2015/07/21 19:20:10 by gjensen          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool	test_islower(t_bool debug);
t_bool	test_isupper(t_bool debug);
t_bool	test_toupper(t_bool debug);
t_bool	test_tolower(t_bool debug);
t_bool	test_puts(t_bool debug);
t_bool	test_putstr(t_bool debug);
...
int main(void)
{
	...
	print_res_test("ft_islower", test_islower);
	print_res_test("ft_isupper", test_isupper);
	print_res_test("ft_toupper", test_toupper);
	print_res_test("ft_tolower", test_tolower);
	print_res_test("ft_puts", test_puts);
	print_res_test("ft_putstr", test_putstr);
...
t_bool	test_islower(t_bool debug)
{
	...
	{
		test = TRUE;
		if (islower(buffer[i]) != ft_islower(buffer[i]))
		{
			ret = FALSE;
			test = FALSE;
		}
		if (debug)
			printf("\t%d | %d | %d | [%s]\n", buffer[i], islower(buffer[i]), ft_islower(buffer[i]), test ? OK : KO);
	}
	return (ret);
}
...
t_bool	test_isupper(t_bool debug)
{
	...
	{
		test = TRUE;
		if (isupper(buffer[i]) != ft_isupper(buffer[i]))
		{
			ret = FALSE;
			test = FALSE;
		}
		if (debug)
			printf("\t%d | %d | %d | [%s]\n", buffer[i], isupper(buffer[i]), ft_isupper(buffer[i]), test ? OK : KO);
	}
	return (ret);
}
...
#define BUFF_SIZE 50
	...
	for (size_t pos = 0; pos < (sizeof(strings) / sizeof(char*)); pos++)
	{
		...
		dup2(out_pipe[1], STDOUT_FILENO);
		close(out_pipe[1]);
		ft_putstr(strings[pos]); /*FT_PUTS CMD*/
		fflush(stdout);
		if(strings[pos])
